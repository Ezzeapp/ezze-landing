...
export default async function HomePage() {
  // Static export: lang defaults to "ru" at build time.
  // Runtime language switching is handled client-side by the Header component.
  const lang: Lang = "ru";
  const t = tr[lang];

  const sections = await getSections("main", lang);
  ...
  const stats = statsContent.items && statsContent.items.length > 0
    ? statsContent.items
